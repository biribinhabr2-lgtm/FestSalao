...
import { ChevronLeft, ChevronRight, Plus, Trash2, Edit3, Clock, Loader2 } from 'lucide-react'
...
export default function Escalas() {
  ...
  const emptyForm = { funcionario_id:'', data: format(new Date(),'yyyy-MM-dd'), turno:'Manhã', horario:'08:00–14:00', setor:'Recreação', tipo:'Normal', observacoes:'' }
  const [form, setForm] = useState(emptyForm)

  const weekDays = eachDayOfInterval({ start: weekStart, end: addDays(weekStart, 6) })

  /* Normaliza: Supabase devolve usuarios: {nome}, local devolve funcionario */
  ...
  function openEdit(item) {
    setEditItem(item)
    setForm({ funcionario_id: item.funcionario_id||'', data: item.data, turno: item.turno, horario: item.horario, setor: item.setor, tipo: item.tipo, observacoes: item.observacoes||'' })
    setModalOpen(true)
  }
    ...
    <div className="page">
      <div className="page-header">
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
          ...
          {isAdmin && <button className="btn btn-primary btn-sm" onClick={()=>openCreate()}><Plus size={14}/> Nova Escala</button>}
        </div>
      </div>
      ...
      {view === 'semana' ? (
          ...
          <div style={{ overflowX:'auto' }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(7,minmax(120px,1fr))', minWidth:700 }}>
              {weekDays.map(day => {
                ...
                return (
                  <div key={day.toString()} style={{ minHeight:160, padding:'10px', borderRight:'1px solid var(--border)', borderBottom:'1px solid var(--border)', background:isToday?'#FFF7F3':'var(--surface)' }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
                      <div>
                        <div style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'.06em', color:'var(--text-3)' }}>{format(day,'EEE',{locale:ptBR})}</div>
                        <div style={{ fontSize:15, fontWeight:800, color:isToday?'var(--accent)':'var(--text)', lineHeight:1 }}>{format(day,'d')}</div>
                      </div>
                      {isAdmin && (
                        <button onClick={()=>openCreate(format(day,'yyyy-MM-dd'))} style={{ width:22,height:22,borderRadius:4,background:'transparent',border:'1.5px dashed var(--border-2)',color:'var(--text-3)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',fontSize:12 }}>+</button>
                      )}
                    </div>
                    <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
                      {dayEscalas.map(es=>(
                        <div key={es.id} onClick={()=>isAdmin&&openEdit(es)} style={{ padding:'4px 7px', borderRadius:4, cursor:isAdmin?'pointer':'default', background:turnoColor[es.turno]||'var(--surface-2)', color:turnoText[es.turno]||'var(--text)', fontSize:10.5, fontWeight:600, lineHeight:1.3 }}>
          ...
          <div style={{ padding:'12px 20px', borderTop:'1px solid var(--border)', display:'flex', flexWrap:'wrap', gap:8 }}>
            {Object.entries(turnoColor).map(([k,v])=>(
              <div key={k} style={{ display:'flex', alignItems:'center', gap:5, fontSize:11.5, color:'var(--text-2)' }}>
                <div style={{ width:10,height:10,borderRadius:2,background:v }}/>{k}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card">
          <div className="table-wrap">
            <table>
              <thead><tr>
                <th>Funcionário</th><th>Data</th><th>Turno</th><th>Horário</th><th>Setor</th><th>Tipo</th>
                {isAdmin && <th style={{ width:80 }}>Ações</th>}
              </tr></thead>
              <tbody>
                {filtered.length === 0 && <tr><td colSpan={7}><div className="empty-state"><div className="empty-icon">📅</div><div className="empty-title">Nenhuma escala encontrada</div></div></td></tr>}
                {filtered.sort((a,b)=>a.data.localeCompare(b.data)).map(es=>(
                  <tr key={es.id}>
                    <td><div style={{ display:'flex', alignItems:'center', gap:8 }}><Avatar nome={nomeFuncionario(es)}/><span style={{ fontWeight:600 }}>{nomeFuncionario(es)}</span></div></td>
                    <td style={{ fontSize:13, color:'var(--text-2)' }}>{format(new Date(es.data+'T00:00'),'dd/MM/yyyy')}</td>
                    <td><span style={{ padding:'3px 8px', borderRadius:99, fontSize:11.5, fontWeight:600, background:turnoColor[es.turno]||'var(--surface-2)', color:turnoText[es.turno]||'var(--text)' }}>{es.turno}</span></td>
                    <td style={{ fontSize:13, color:'var(--text-2)', display:'flex', alignItems:'center', gap:5 }}><Clock size={12}/>{es.horario}</td>
      ...
      <Modal open={modalOpen} onClose={()=>setModalOpen(false)} title={editItem?'Editar Escala':'Nova Escala'}
        footer={<><button className="btn btn-secondary btn-sm" onClick={()=>setModalOpen(false)}>Cancelar</button><button className="btn btn-primary btn-sm" onClick={handleSave}>{editItem?'Salvar':'Adicionar'}</button></>}
      >
