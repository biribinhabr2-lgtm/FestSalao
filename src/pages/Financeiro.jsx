...
import { Plus, TrendingUp, TrendingDown, DollarSign, Trash2, Filter, Download } from 'lucide-react'
import Modal from '../components/ui/Modal'
import { useToast } from '../components/ui/Toast'
import { mockMovimentacoes as initMov, mockCategorias } from '../data/mockData'
...
export default function Financeiro() {
  ...
  const [movs, setMovs] = useState(initMov)
  ...
  function handleSave() {
    ...
    setMovs(s=>[{ id:`m${Date.now()}`, ...form, valor:parseFloat(form.valor), usuario:'Admin Demo' }, ...s])
    toast.success('Movimentação registrada!')
    setModalOpen(false)
  }
  function handleDelete(id) {
    if (!confirm('Excluir esta movimentação?')) return
    setMovs(s=>s.filter(m=>m.id!==id))
    toast.success('Movimentação excluída.')
  }

  const fmt2 = (v) => `R$ ${v.toLocaleString('pt-BR',{minimumFractionDigits:2})}`
    ...
    <div className="page">
      ...
      <div className="stat-grid" style={{ gridTemplateColumns:'repeat(3,1fr)', marginBottom:20 }}>
        <div className="stat-card" style={{ borderTop:'3px solid var(--green)' }}>
          ...
          <div>
            <div className="stat-value" style={{ color:'var(--green)', fontSize:22 }}>{fmt2(totals.entradas)}</div>
        ...
        <div className="stat-card" style={{ borderTop:'3px solid var(--red)' }}>
          ...
          <div>
            <div className="stat-value" style={{ color:'var(--red)', fontSize:22 }}>{fmt2(totals.saidas)}</div>
        ...
        <div className="stat-card" style={{ borderTop:`3px solid ${totals.saldo>=0?'var(--green)':'var(--red)'}` }}>
          ...
          <div>
            <div className="stat-value" style={{ color:totals.saldo>=0?'var(--green)':'var(--red)', fontSize:22 }}>{fmt2(totals.saldo)}</div>
      ...
      <div className="two-col">
        ...
        <div>
          ...
          <div className="card">
            {filtered.length===0 ? (
              <div className="empty-state"><div className="empty-icon">💰</div><div className="empty-title">Nenhuma movimentação</div><div className="empty-desc">Nenhuma movimentação encontrada para os filtros selecionados.</div></div>
            ) : (
                    ...
                    {filtered.map(m=>(
                      <tr key={m.id}>
                        <td>
                          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                            <div style={{
                              ...
                              background:m.tipo==='entrada'?'var(--green-light)':'var(--red-light)',
                        ...
                        <td style={{ fontSize:13, color:'var(--text-2)' }}>
                          {format(new Date(m.data+'T00:00'),'dd/MM/yy')}
                        </td>
                        ...
                        <td>
                          <button className="btn btn-icon btn-danger btn-sm" onClick={()=>handleDelete(m.id)}><Trash2 size={12}/></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                ...
                return (
                  <div key={cat.nome} style={{ padding:'10px 20px', borderBottom:'1px solid var(--border)' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:5 }}>
                      ...
                      <span style={{ fontSize:13, fontWeight:700, color:cat.tipo==='entrada'?'var(--green)':'var(--red)' }}>
                        {fmt2(cat.total)}
                      </span>
                    </div>
                    <div className="progress-bar">
                      <div className={`progress-fill ${cat.tipo==='entrada'?'green':''}` } style={{ width:`${pct}%`, background:cat.tipo==='saida'?'var(--red)':undefined }}/>
                    </div>
          ...
          <div className="card">
            <div className="card-header">
              <span className="card-title">Fechamento de Caixa</span>
            </div>
            <div style={{ padding:'16px 20px', display:'flex', flexDirection:'column', gap:10 }}>
              <div style={{ fontSize:13, color:'var(--text-2)', lineHeight:1.5 }}>
                Registre o fechamento do caixa do dia com o saldo físico conferido.
              </div>
              <div className="form-group">
                <label className="form-label">Saldo físico conferido</label>
                <input type="number" step="0.01" className="form-input" placeholder="0,00" />
              </div>
              <div className="form-group">
                <label className="form-label">Observações</label>
                <textarea className="form-input" style={{ minHeight:60 }} placeholder="Divergências, observações…" />
              </div>
              <button className="btn btn-primary btn-sm" onClick={()=>toast.success('Fechamento de caixa registrado!')}>
      ...
      <Modal open={modalOpen} onClose={()=>setModalOpen(false)} title="Nova Movimentação"
        footer={<>
          ...
          <button className="btn btn-primary btn-sm" onClick={handleSave}>Registrar</button>
        </>}
      >
          ...
          {['entrada','saida'].map(t=>(
            <button key={t} type="button"
              ...
              style={{ flex:1, justifyContent:'center',
                background: form.tipo===t?(t==='entrada'?'var(--green)':'var(--red)'):undefined,
                borderColor: form.tipo===t?(t==='entrada'?'var(--green)':'var(--red)'):undefined,
              }}
        ...
        <div className="form-group">
          ...
          <input className="form-input" placeholder="Ex: Festa Sofia — pacote premium" value={form.descricao} onChange={e=>setForm(f=>({...f,descricao:e.target.value}))} />
        </div>
        <div className="form-row">
          <div className="form-group">
            <label className="form-label">Valor (R$) <span>*</span></label>
            <input type="number" step="0.01" min="0" className="form-input" placeholder="0,00" value={form.valor} onChange={e=>setForm(f=>({...f,valor:e.target.value}))} />
          </div>
          <div className="form-group">
        ...
        <div className="form-group">
          <label className="form-label">Observações</label>
          <textarea className="form-input" style={{ minHeight:60 }} value={form.obs} onChange={e=>setForm(f=>({...f,obs:e.target.value}))} placeholder="Informações adicionais…"/>
        </div>
      </Modal>
    </div>
  )
